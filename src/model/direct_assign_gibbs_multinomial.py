...
class DirectAssignmentMultinomial(DirectAssignment):
    def __init__(self, model, observations, dir0=1):
        ...
        self.y_sum = np.array([observations[0]])
        self.m_multi = len(self.observations[0]) # m_multi
        self.n_multi = sum(self.observations[0]) # n_multi

        # Multinomial params
        self.dir0 = dir0 * np.ones(self.observations.shape[1]) # shape[1] is number of columns
        self.dir0sum = np.sum(self.dir0)


    def emission_pdf(self):
        return lambda x: np.exp(np.real(
            (ssp.loggamma(self.dir0sum + self.y_sum.sum(axis=1)) - ssp.loggamma(
                self.dir0sum + self.y_sum.sum(axis=1) + self.n_multi)) + np.sum(
                ssp.loggamma(self.dir0 + x + self.y_sum), axis=1) - np.sum(
                ssp.loggamma(self.dir0 + self.y_sum), axis=1))), lambda x: np.exp(
    ...
    def sample_one_step_ahead(self, t):
        ...
        if have_new_state:
            ...
            self.y_sum = np.vstack((self.y_sum, np.zeros((1, self.m_multi))))

            # Add a new state
            self.K += 1

        self.transition_count[last_state, self.hidden_states[t]] += 1
        self.y_sum[self.hidden_states[t]] += self.observations[t]
    ...
    def update_K(self):
        ...
        self.y_sum = self.y_sum[remain_index]
    ...
    def compute_log_marginal_likelihood(self, test_observations, start_point=-1):
        ...
        yt_dist = (ssp.loggamma(self.dir0sum + self.y_sum.sum(axis=1)) - ssp.loggamma(
            self.dir0sum + self.y_sum.sum(axis=1) + test_n_multi)) - np.sum(ssp.loggamma(self.dir0 + self.y_sum), axis=1)
        ...
        single_term = np.vstack((self.dir0 + self.y_sum, self.dir0))
