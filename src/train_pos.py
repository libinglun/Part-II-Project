    ...
    for iteration in tqdm.tqdm(range(iterations), desc="training model:"):
        ...
        if iteration == 0:
            for index in range(len(train_sentences)):
                # model.initialise_first_state(index)
            ...
            for index in range(len(train_sentences)):
                # model.sample_hidden_states_on_next_state(index, 0)
            ...
            # print("alpha: ", model.src.alpha, "gamma: ", model.src.gamma)

            # if iteration % 10 == 0:
            #     hidden_states_sample.append(model.hidden_states.copy())
            #     hyperparams_sample.append(np.array([model.src.alpha, model.src.gamma]))
            # print("shape:" , model.token_state_matrix.shape)

        # print(model.token_state_matrix.dtype)
        ...
        if iteration % 10 == 0:
            ...
            model.sample_transition_distribution()
            ...
            for test_sentence in test_sentences:
                _, loglik = model.compute_log_marginal_likelihood(test_sentence)
                sum_loglik += loglik
